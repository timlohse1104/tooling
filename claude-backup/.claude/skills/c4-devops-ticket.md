...
- Kein Match → Nutzer = **Tim Köster** (Default, kein Rückfragen)
...
1. Sprint → Aktueller Sprint | Infrastruktur Backlog Sprint
...
*(Komponente ist immer **Infrastruktur** – wird nicht abgefragt)*
...
```
Falls "Aktueller Sprint" gewählt:
  mcp__claude_ai_Atlassian__searchJiraIssuesUsingJql
  JQL: project = DO AND sprint in openSprints() ORDER BY created DESC
  fields: ["customfield_10020"]
  → Lese aus dem ersten Issue das Feld customfield_10020 (Array)
  → Extrahiere daraus den Wert von "id" als **ganze Zahl** (z.B. 42, nicht "42")
  → Diese Zahl wird als Sprint-ID an createJiraIssue übergeben

Falls "Infrastruktur Backlog Sprint" gewählt:
  mcp__claude_ai_Atlassian__searchJiraIssuesUsingJql
  JQL: project = DO AND sprint = "Infrastruktur Backlog" ORDER BY created DESC
  fields: ["customfield_10020"]
  → Lese aus dem ersten Issue das Feld customfield_10020 (Array)
  → Extrahiere daraus den Wert von "id" als **ganze Zahl**
  → Falls kein Ergebnis: Sprint-Feld beim createJiraIssue weglassen
```

**Wichtig:** Der Sprint-Wert muss beim `createJiraIssue`-Aufruf als Integer übergeben werden:
```json
"customfield_10020": 42
```
**Nicht** als String `"42"` und **nicht** als Objekt `{"id": 42}` – nur die nackte Zahl.
...
- Komponente ist immer **Infrastruktur** – nie abfragen
- Sprint-Auswahl: nur **Aktueller Sprint** oder **Infrastruktur Backlog Sprint** – kein Freitext
- Nutzer-Identifikation: **kein Rückfragen**, bei unbekanntem Git-User Default = Tim Köster
