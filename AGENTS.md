...
- `lieselotte` models: `Qwen3.6-27B`, `Qwen3.6-35B-A3B`, `gemma-4-26B-A4B`, `ornith-1.0-9b`, `ornith-1.0-35b`, `Qwen-AgentWorld-35B-A3B`, `gemma-4-31B`, `Laguna-XS-2.1`. `hermine` models: `Qwen3.6-27B`, `Qwen3.6-35B-A3B`, `gemma-4-31B`, `Laguna-XS-2.1` — these four are the active sections in hermine's `presets/models.ini`; the remaining four are `;`-commented there but stay in both providers' `whitelist` in `opencode.jsonc`, so the whitelist is a superset of what the router actually serves.
...
- Laguna XS 2.1 (poolside, OpenMDW-1.1, agentic coding): 33B-A3B MoE (arch `laguna`, 40 layers, 256 experts + 1 shared, native 262k ctx). Mixed attention layout — 10 global-attention layers + 30 sliding-window (512) layers in a 3:1 ratio with sigmoid gating and per-layer rotary scales — so only a quarter of the layers carry a growing KV-cache and long contexts stay cheap on VRAM. Built for long-horizon agent work (SWE-bench Verified 70.9, SWE-bench Multilingual 63.1, Terminal-Bench 2.0 37.5). Reasoning is interleaved between tool calls and **on by default**; the model expects *preserved thinking* (prior assistant `reasoning_content` kept in the message history) → `jinja = true` + `reasoning-format = auto`, and clients that drop thinking blocks will see it stop reasoning in follow-up steps. Upstream llama.cpp support merged via PR #25165 (2026-07-22). Preset uses bartowski's imatrix `IQ4_XS` (18.2 GB) for KV/compute headroom on a 24 GB card. Vendor sampling (the only preset the card documents, used for all published benchmarks): `temp 1.0`, `top-p 1.0`, `top-k 20`. A DFlash speculator exists (`poolside/Laguna-XS-2.1-DFlash`) and the build advertises `--spec-type draft-dflash`, but no GGUF in llama.cpp's own `dflash` arch was found — so no *draft-model* speculator is wired. It does run `spec-type = ngram-mod`, which needs no draft model at all, making that the only speculator this model can currently use.
...
- `spec-type` carries `ngram-mod` in **every** section of both `presets/models.ini` (4 active) and `presets/models.example.ini` (9) — as `draft-mtp,ngram-mod` where an MTP drafter exists, as bare `ngram-mod` where none does (gemma-4-26B-A4B, both Ornith, Qwen-AgentWorld, Laguna, Unlimited-OCR). It needs no draft model and no VRAM, and speculative decoding is lossless, so a bad fit costs speed and never output quality. The gain is **unmeasured on both machines** — adopted from countzero's 24 GB preset on mechanism. Expect the most from Unlimited-OCR (greedy decoding makes drafts all-or-nothing, and scanned documents repeat structure) and from file-rewrite-heavy agent turns; expect nothing on novel prose, where the 48-token minimum chain length suppresses the draft entirely. `cache-ram = 16384` is set on hermine's two Qwen3.6 sections (27B and 35B-A3B) and nowhere else: the default is 8192, the box has 39 GB, and the value applies **per loaded model**, so raising `models-max` above 1 multiplies it.
